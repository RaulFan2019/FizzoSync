...
public class ConnectEntity {
    ...
    public ConnectEntity(Context context, final String mac, final BluetoothAdapter mAdapter) {
        ...
        Log.e(TAG,"new ConnectEntity mAddress:" + mAddress);
        ...
        try {
            ...
            Log.e(TAG,"ConnectEntity IllegalArgumentException:" + ex.getMessage());
    ...
    private synchronized void onConnectionStateChange(final BluetoothGatt gatt, final int status, final int newState) {
        // 连接失败判断
        if (status != BluetoothGatt.GATT_SUCCESS) {
            Log.e(TAG, "<" + mAddress + ">" + "status !GATT_SUCCESS ,mNeedConnect:" + mNeedConnect);
            mSyncHrState = false;
            //若需要重连
            if (mNeedConnect) {
                mIsConnected = false;
        ...
        if (newState == BluetoothProfile.STATE_CONNECTED) {
            Log.e(TAG, "<" + mAddress + ">" + "Connected to GATT server.");
            Log.e(TAG, "<" + mAddress + ">" + "Attempting to start service discovery");
            destroyHandler();
            ...
            Log.e(TAG, "<" + mAddress + ">" + "Disconnected from GATT server.");
            mSyncHrState = false;
            if (!mNeedConnect) {
    ...
    private synchronized void onCharacteristicReadFail(final BluetoothGattCharacteristic characteristic) {
        Log.e(TAG, "onCharacteristicReadFail");
    }
...
//        Log.e(TAG, "onCharacteristicChanged");
    ...
    private void analysisHrTotalData(final byte[] data) {
        ...
        Log.e(TAG,"analysisHrTotalData:" + "mSyncHrHistorySize:" + mSyncHrItemsSize + "," + mSyncHrItemsSize);
    ...
    private void analysisHrHeaderData(final byte[] data) {
        ...
        //ee fa fe 40 0c 00 49 74 42 59 a5 00 00 00 00 00 00 10 00 00
        byte[] startTimeB = new byte[]{data[5], data[4], data[3], data[2]};
        Log.e(TAG, "analysisHrHeaderData startTime:" + ByteU.bytesToLong(startTimeB));
    ...
    private void analysisDeleteOneHrData() {
        Log.e(TAG, "mSyncHrState:" + mSyncHrState);
        if (mSyncHrState) {
    ...
    private synchronized void writeCharacteristicSuccess(final BluetoothGattCharacteristic characteristic) {
        Log.e(TAG, "writeCharacteristicSuccess");
    }
    ...
    private synchronized void writeCharacteristicFail(final BluetoothGattCharacteristic characteristic) {
        Log.e(TAG, "writeCharacteristicFail");
    ...
    private void writeGetHrDataTotalRequest() {
        Log.e(TAG,"writeGetHrDataTotalRequest");
    ...
    private void writeDeleteOneHrDataRequest(long startTime) {
        Log.e(TAG, "writeDeleteOneHrDataRequest");
