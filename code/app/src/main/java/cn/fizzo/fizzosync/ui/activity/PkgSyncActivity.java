...
import cn.fizzo.fizzosync.entity.adapter.SyncDeviceEntity;
...
import cn.fizzo.fizzosync.ui.adapter.SyncDeviceListAdapter;
...
public class PkgSyncActivity extends BaseActivity implements ConnectListener, SyncHrDataListener {
    ...
    /* local data */
    List<SyncDeviceEntity> mSyncList = new ArrayList<>();

    private SyncDeviceListAdapter mAdapter;
    private SyncDeviceEntity mCurrSyncEntity;
...
//                    Log.v(TAG,"mSyncTimer:" + mSyncTimer);
    ...
    public void completeOneHrHistory(SyncHrDataEntity entity) {
        Log.e(TAG,"completeOneHrHistory");
        mHandler.removeMessages(MSG_ADD_SYNC_TIMER);
    ...
    protected void initData() {
        mRotateAnim = (RotateAnimation) AnimationUtils.loadAnimation(PkgSyncActivity.this, R.anim.rotating);
        List<GetDeviceListRE.HrdevicesBean> mSelectList = (List<GetDeviceListRE.HrdevicesBean>)
                getIntent().getExtras().getSerializable("device");
        mDialogBuilder = new DialogBuilder();
        for (GetDeviceListRE.HrdevicesBean trainee : mSelectList) {
            mSyncList.add(new SyncDeviceEntity(trainee, SyncTraineeEntity.SYNC_NONE));
        }
    }
    ...
    protected void initViews(Bundle savedInstanceState) {
        mAdapter = new SyncDeviceListAdapter(PkgSyncActivity.this, mSyncList);
        ...
        list.setOnItemClickListener(new AdapterView.OnItemClickListener() {
            ...
            public void onItemClick(AdapterView<?> adapterView, View view, int i, long l) {
                SyncDeviceEntity syncTraineeEntity = mSyncList.get(i);
     ...
     * @param syncEntity
     */
    private void startSync(final SyncDeviceEntity syncEntity, boolean lastFinish) {
        if (lastFinish) {
        ...
        mCurrSyncEntity = syncEntity;
        tvDevice.setText(mCurrSyncEntity.trainee.macaddr);
        tvName.setText("(" + mCurrSyncEntity.trainee.hrdevicenumber + ")" + mCurrSyncEntity.trainee.name);
        ...
        Fw.getManager().addNewConnect(mCurrSyncEntity.trainee.macaddr);
        mHandler.sendEmptyMessageDelayed(MSG_ADD_CONNECT_TIMER, 1000);
    }
...
//        检查记录是否合理
        ...
        for (SyncHrItemEntity item : entity.itemEntities) {
            ...
            if (item.timeOffSet / 60 > splitId) {
                ...
                for (int j = 0, sizeJ = ((int) item.timeOffSet / 60) - splitId; j < sizeJ; j++) {
                    int percent = avgHeartbeat * 100 / 200;

                    avg_bpms += avgHeartbeat + ",";
                    avg_efforts += percent + ",";
                    hr_zones += ZoneU.getZone(percent) + ",";
                    effort_points += ZoneU.getMinutesEffortPoint(avgHeartbeat, 200) + ",";
                    calories += CalorieU.getCalorie(avgHeartbeat) + ",";
                    splitId++;
                }
            }
            splitHrSize++;
        ...
        x.task().post(new Runnable() {
            @Override
            public void run() {
                RequestParams params = RequestParamsBuilder.buildUploadMinuteEffortArray(PkgSyncActivity.this, UrlConfig.UPLOAD_MINUTE_EFFORT_ARRAY,
                        TimeU.formatDateToStr(new Date(entity.startTime * 1000), TimeU.FORMAT_TYPE_1), minutes, mCurrSyncEntity.trainee.macaddr,
    ...
    private void findNextSyncTrainee(boolean lastFinish) {
        ...
        for (int i = 0, size = mSyncList.size(); i < size; i++) {
            if (mSyncList.get(i).trainee.macaddr.equals(mCurrSyncEntity.trainee.macaddr)) {
                if (!lastFinish) {
                    SyncDeviceEntity entity = mSyncList.get(i);
    ...
    private void showSyncFailDialog() {
        mDialogBuilder.showSingleMsgDialog(PkgSyncActivity.this, "同步失败!", "请确认：\n\n" +
                "1. 手环已打开或处于充电状态\n\n" +
                "2. 手环需尽量靠近手机");
    }
}
